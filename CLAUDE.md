...
- **`.chezmoiscripts/`** — all `run_*` scripts live here (chezmoi's special directory for scripts that don't create a corresponding dir in `$HOME`).
  - `run_check-tools.sh.tmpl` — Linux-only, checks for required CLI tools (rg, fzf, bat, eza).
  - `run_check-claude-code.sh.tmpl` — checks for Claude Code CLI.
  - `run_check-zellij.sh.tmpl` — checks for zellij terminal multiplexer.
  - `run_set-default-shell.sh.tmpl` — Linux-only, sets zsh as default shell.
